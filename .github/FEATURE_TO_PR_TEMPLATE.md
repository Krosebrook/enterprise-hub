...
**Key Patterns**:
- **Auth**:  Base44 authentication with AuthContext (check src/lib/AuthContext.jsx)
- **Validation**: Manual checks and Zod (zod package available but not widely used yet)
- **Styling**: Tailwind utility classes with custom theme (see tailwind.config.js)
- **Components**: Radix UI primitives + custom wrappers in src/components/ui
- **Error Handling**: Console logging, React error boundaries
- **State Management**: React Context + React Query for server state

**Known Constraints**:
- ⚠️ **No tests yet**: Testing infrastructure needs to be added (no test runner configured)
- ⚠️ **JavaScript only**: No TypeScript (except 1 backend function: functions/generateTerraform.ts)
- ⚠️ **8 security vulnerabilities**: Run `npm audit` to see current vulnerabilities (6 moderate, 2 high)
- ⚠️ **54 ESLint errors**: Unused imports (use `npm run lint:fix` to auto-fix)

**Build Commands**:
```bash
npm run dev       # Development server
npm run build     # Production build
npm run lint      # ESLint check (with --quiet flag)
...
**Helpful Context**:
- 12 application pages (Dashboard, Agents, Architectures, Policies, Users, Compliance, Observability, Costs, AuditLog, etc.)
- ~59 component files in src/components
- 1 backend function (Terraform code generation)
- Focus on enterprise AI governance, compliance, and architecture management
- RBAC (Role-Based Access Control) system with PermissionGate components
- Pages include: Agent management, Architecture Designer, Policy enforcement, Compliance tracking, Cost monitoring, Audit logging
...
Outcome: Users can search AI agents by name/description and filter by 
status, cost, and compliance score. Results display in real-time with 
highlighting. 
...
- [ ] Filter by status (dropdown), cost range (slider), compliance score
...
If stuck or unsure: 
1. **Check documentation**:  README, .github/copilot-instructions.md
